    ...
    <>
      ...
      {issued ? (
        ...
        */
        <div className="mb-6 rounded-xl border border-success-700/30 bg-success-50 p-4">
          ...
          <p className="text-display-xs mt-1 font-mono tracking-wider">{issued}</p>
          <p className="text-body-xs mt-2 text-foreground-muted">
            Copy this now and send it to the recipient. Only a hash is stored, so it cannot be shown
            again — not to you, not to anyone. Reload this page and it is gone.
          </p>
        </div>
      ) : null}
      ...
      <div className="space-y-6">
        <AdminCard title="Coupons" description={`${coupons.length} codes`}>
          {coupons.length === 0 ? (
            <p className="py-6 text-center text-body-sm text-foreground-subtle">No coupons yet.</p>
          ) : (
                  ...
                  {coupons.map((coupon) => {
                    return (
                      <tr key={coupon.id} className="border-b border-border last:border-0">
                        <td className="text-body-xs py-2.5 pr-3 font-mono font-medium">
                          {coupon.code}
                        </td>
                        ...
                        <td className="text-body-xs py-2.5 text-right text-foreground-subtle">
          ...
          actions={
            canIssueCredit ? (
              <details className="relative">
                <summary className="text-body-xs cursor-pointer rounded-lg bg-accent px-3 py-1.5 font-medium text-white">
                  Issue a card
                </summary>
                <form
                ...
                >
                  <div>
                    <label htmlFor="gc-amount" className="text-body-xs mb-1 block font-medium">
                      Amount ($)
                    </label>
                    <input
                      id="gc-amount"
                  ...
                  <div>
                    <label htmlFor="gc-email" className="text-body-xs mb-1 block font-medium">
                      Recipient email
                    </label>
                    <input
                      id="gc-email"
                  ...
                  <div>
                    <label htmlFor="gc-expires" className="text-body-xs mb-1 block font-medium">
                      Expires
                    </label>
                    <input
                      id="gc-expires"
                  ...
                  <div>
                    <label htmlFor="gc-note" className="text-body-xs mb-1 block font-medium">
                      Note
                    </label>
                    <input
                      id="gc-note"
                  ...
                  <button
                    type="submit"
                    className="text-body-xs w-full rounded-lg bg-accent px-3 py-2 font-medium text-white hover:bg-accent-hover"
                  >
                    Issue
                  </button>
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-body-xs truncate text-foreground-subtle">
                      {card.issuedToEmail ?? 'Unassigned'} · issued {formatDate(card.createdAt)}
                  ...
                  <div className="flex shrink-0 items-center gap-3">
                    ...
                    <span className="text-body-sm font-medium tabular-nums">
                      ...
                      <span className="text-body-xs ml-1 font-normal text-foreground-subtle">
            ...
            <ul className="divide-y divide-border">
              {referrals.map((code) => (
                ...
                >
                  ...
                  <span className="shrink-0 text-body-sm tabular-nums">{code.uses} uses</span>
                </li>
              ))}
            </ul>
          )}
